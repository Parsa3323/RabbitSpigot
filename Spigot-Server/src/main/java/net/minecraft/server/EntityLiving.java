...
//        }
        if (this.random.nextDouble() >= this.getAttributeInstance(GenericAttributes.c).getValue()) {
            ...
            float f1 = MathHelper.sqrt(d0 * d0 + d1 * d1);
            double friction = RabityConfig.getKnockbackValue("friction");
            double horizontal = RabityConfig.getKnockbackValue("horizontal");
            double vertical = RabityConfig.getKnockbackValue("vertical");
            double verticalLimit = RabityConfig.getKnockbackValue("verticalLimit");
            double extraHorizontal = RabityConfig.getKnockbackValue("extraHorizontal");
            double extraVertical = RabityConfig.getKnockbackValue("extraVertical");

            // Apply knockback with configurable values
            this.motX /= friction;
            this.motY /= friction;
            this.motZ /= friction;
            this.motX -= (d0 / f1) * horizontal;
            this.motY += vertical + extraVertical;
            this.motZ -= (d1 / f1) * horizontal;

            // Limit vertical knockback
